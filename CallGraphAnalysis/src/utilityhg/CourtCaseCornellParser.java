...
public class CourtCaseCornellParser {
	public static TreeSet<String> caseNames;
	public static HashSet<String> caseIDs;
	public static HashMap<String, String> caseNameID;
	public static HashMap<String, String> caseNameMatching;
	public static String caseTopic = "pension";

	public static void htmlParse(String filePath) throws Exception {
		Scanner scanner = new Scanner(new File(filePath));
		PrintWriter pw = new PrintWriter(new File(
				"supremecourt_networks//case-" + caseTopic + "-name.txt"));
	...
	public static void findUSCaseNum(String filePath) throws Exception {
		PrintWriter pw = new PrintWriter(new File(
				"supremecourt_networks//case-" + caseTopic + "-id-parsed.txt"));
		Scanner scanner = new Scanner(new File(filePath));
		...
		while (scanner.hasNextLine()) {
			...
			for (String c : caseNames) {
				...
				for (index = 0; index < tokens.length; ++index) {
					...
					if (s.length() < 2)
						continue; // skip space and single characters
					if (s.equals("of"))
						continue;
					if (s.equals("for"))
						continue;
					...
					if (matchIndex > -1) {
						...
						// if (line.charAt(matchIndex + s.length()) != ' ' &&
						// line.charAt(matchIndex + s.length()) != ',')
						// continue; // not exact match
						if (flg == 0) {
							++kntPrefix;
						} else {
	...
	public static void loadCuratedCaseIDs() throws Exception {
		caseIDs = new HashSet();
		Scanner scanner = new Scanner(new File("supremecourt_networks//case-"
				+ caseTopic + "-id-curated.txt"));
